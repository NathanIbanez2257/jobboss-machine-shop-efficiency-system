...
public class machinist {
    public ReportResult runReport(String startDate, String endDate) throws Exception {
        ...
        System.out.print("\nFinished Script\n");
        return new ReportResult(finalList, jobNumbers);
    }
    ...
    private static String getJobNumber(String employeeCode, String ticketDate, String apiKey) throws Exception {
        String urlString = String.format(
                "https://.mye2shop.com/api/v1/time-ticket-details?employeeCode=%s&fields=employeeCode,comments,employeeName,jobNumber,workCenter,piecesFinished,piecesScrapped,cycleTime,ticketDate&ticketDate=%s",
    ...
    private static List<String[]> convertJSONToTableData(List<String[]> mainList, String jsonData) {
        ...
        if (dataArray.length() != 1) {
            for (int x = 0; x < dataArray.length(); x++) {
                JSONObject rowData = dataArray.getJSONObject(x);
                data.add(rowData);
            }
            for (JSONObject x:data) {
                boolean check = false;
                boolean preAdded = false;
                for (JSONObject dup: finalData){
                    if(dup.get("jobNumber").equals(x.get("jobNumber"))) {
                        check = true;
                    }
                }
                for (int i = 1; i < data.size(); i++) {
                    if (!(x.getString("jobNumber").equals(data.get(i).getString("jobNumber")))) {
                        if(data.size() == 2) {
                            System.out.print("\n\n2 ELEMENTS ONLY\n\n");
                            finalData.add(x);
                        ...
                        if (!check && data.indexOf(x) != data.indexOf(data.get(i)) ){
                            x.put("piecesFinished", x.getDouble("piecesFinished") + data.get(i).getDouble("piecesFinished"));
                            x.put("piecesScrapped", x.getDouble("piecesScrapped") + data.get(i).getDouble("piecesScrapped"));
                            x.put("cycleTime", (Math.round(x.optDouble("cycleTime", 0.0) * 10) / 10.0) +
                                    (Math.round(data.get(i).optDouble("cycleTime", 0.0) * 10) / 10.0));
                        } else {
                            continue;
                        }
                    }
                    int lastCheck = i + 1;
                    if (lastCheck == data.size() && !preAdded && !check) {
                        finalData.add(x);
                    }
                }
            }
        ...
        for (JSONObject rowData: finalData) {
            ...
            row[7] = String.valueOf(Math.round(rowData.optDouble("cycleTime", 0.9999999) * 10) / 10.0); // Round to the nearest tenth
